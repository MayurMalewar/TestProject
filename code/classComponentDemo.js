 ...
 */
 ...
 //     setShow(true)
 //   };
 let data = step.POWER.task[0].optFieldDetails;
 class App extends Component {
   constructor(props) {
     ...
     this.state = {
       date: moment().toDate(),
       show: false,
       pickItem: { item: null, index: null, forcast: null },
       stepValue: data,
   ...
   onChange = (event, selectedDate) => {
     ...
     let { item, index, forcast } = this.state.pickItem;
     let fdata = '';
     // console.log('item.actualAndForecast[forcast].fieldValue: ',item.actualAndForecast[forcast].fieldValue);
     if (forcast) {
       fdata = JSON.parse(item.actualAndForecast[forcast].fieldValue)
     ...
     let copystepValue = [...stepValue];
     copystepValue[index] = this.state.pickItem(item);
     this.setState({stepValue: this.copystepValue});
     // console.log('copystepValue:', copystepValue, "item", item);
   };
   showDatepicker = (item, index, forcast) => {
     // console.log("values:", stepValue, "=>", item, '=>', index);
     this.setState({
       // pickItem({
       //   item: item,
       ...
       //   forcast: forcast
       // }),
       pickItem: {item:item, index:index, forcast: forcast},
       show: true
     });
   ...
   render() {
     return (
       ...
       <SafeAreaView style={{ padding: 10, flex: 1 }}>
         {/* {console.log('entered')} */}
         {this.state.stepValue ? this.state.stepValue.map((item, index) =>
             ...
             {item.subType == 'multiSelect' ?
               ...
               : item.subType == 'Actual & Forecast' ?
                 <View style={{ flexDirection: "row", flex: 2 }}>
                   <TouchableOpacity onPress={() => { item.isVisibleToVendor == 'EDIT' ? this.showDatepicker(item, index, 1) : null }} style={{ flex: 1, borderRighttWidth: 1, alignItems: "center", justifyContent: "center", flexDirection: "row", backgroundColor: item.isVisibleToVendor == 'EDIT' ? '#ffffff' : '#e6e6e6' }}>
                   ...
                   <TouchableOpacity onPress={() => {item.isVisibleToVendor == 'EDIT' ? this.showDatepicker(item, index, 1) : null }} style={{ flex: 1, borderLeftWidth: 1, alignItems: "center", justifyContent: "center", flexDirection: "row", backgroundColor: item.isVisibleToVendor == 'EDIT' ? '#ffffff' : '#e6e6e6' }}>
